...
bool move_media(char* old_location, char* ext, int log_fd) {
    ...
    // For JPG files, build a custom struct with my EXIF utility.
...
void create_dir(int year, int month, char* friendly_dirname, char* friendly_subdir) {
    ...
    DIR* try_root_dir = opendir(ROOT_DIRNAME);
    if (!try_root_dir) {
        mkdir(ROOT_DIRNAME, 0700);
    } else {
        closedir(try_root_dir);
    }
    
    DIR* try_new_dir = opendir(friendly_dirname);

    // create the new directory
    if (!try_new_dir) {
        mkdir(friendly_dirname, 0700);
    } else {
        closedir(try_new_dir);
    }
}
